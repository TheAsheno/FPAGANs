...
class CACD(data.Dataset):
    def __init__(self, split="train", transforms=None, label_transforms=None):
        ...
        with open(label_pair_root,'r', encoding='utf-8') as f:
            lines=f.readlines()
        ...
        for i in range(len(group_lists)):
            with open(group_lists[i], 'r', encoding='utf-8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            group_images = []
            for l in lines:
        ...
        if self.split == "train":
            self.source_images = []
            with open(os.path.join(list_root, 'train.txt'), 'r', encoding='utf-8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
            ...
            self.source_images = []
            with open(os.path.join(list_root, 'test.txt'), 'r', encoding='utf-8') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
